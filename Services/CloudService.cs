...
public class CloudService
{
        ...
        bool isInMeeting = false, string meetingApp = "")
    {
        ...
        try
        {
            ...
            // RPC payload — parameter names match the SQL function exactly
            var payload = new
            {
    ...
    /// <summary>
    /// Backwards-compat shim — older callers used SendStatusAsync. Forward
    /// it to the unified call so they keep working without changes.
    /// </summary>
    ...
    // -----------------------------------------------------------------------
    // Reads — query the v_live_users view directly.
    // -----------------------------------------------------------------------
        ...
        GetActivityByDateAsync(DateTime date)
    {
        ...
        try
        {
            ...
            {
                var userObj = r.TryGetProperty("users", out var u) && u.ValueKind == JsonValueKind.Object
                    ? u
                    : default;
    ...
    private static List<CloudSessionInfo> ParseLiveUsers(string json)
    {
        ...
        foreach (var r in doc.RootElement.EnumerateArray())
        {
            ...
            else if (r.TryGetProperty("open_projects", out var opStr) && opStr.ValueKind == JsonValueKind.String)
            {
                // Defensive: some PostgREST configurations return jsonb as a string
                try
                {
                    using var inner = JsonDocument.Parse(opStr.GetString() ?? "[]");
                    foreach (var p in inner.RootElement.EnumerateArray())
                    {
                        openProjects.Add(new RevitProjectInfo
                        {
                            ProjectName  = p.TryGetProperty("projectName", out var n) ? n.GetString() ?? "" : "",
