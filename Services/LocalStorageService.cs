...
public class LocalStorageService
{
    ...
    public List<DailyWorkRecord> GetRecordsForDateRange(DateTime startDate, DateTime endDate) =>
        _cache.DailyRecords.Where(r => r.Date.Date >= startDate.Date && r.Date.Date <= endDate.Date).ToList();

    public DailyWorkRecord? GetUserDailyRecord(string machineId, string userId, DateTime date) =>
        _cache.DailyRecords.FirstOrDefault(r => r.MachineId == machineId && r.UserId == userId && r.Date.Date == date.Date);
    ...
    private static readonly HashSet<string> InvalidProjectNames =
        new(StringComparer.OrdinalIgnoreCase) {
            "Home","Start","Revit","Recent","New","Open","Models","Families","Templates",
            "Samples","Autodesk","Architecture","Structure","MEP","Revit Session" };
        ...
        string projectName, string application, string revitVersion = "")
    {
        ...
        var existing = _cache.ActiveSessions.FirstOrDefault(s =>
            s.MachineId == machineId && s.UserId == userId && s.ProjectName == projectName &&
            s.IsActive && s.StartTime.Date == today);
        if (existing != null) return;

        var cutoff = DateTime.Now.AddHours(-24);
        var stale = _cache.ActiveSessions
            .Where(s => (s.StartTime < cutoff || s.StartTime.Date < today) && s.IsActive).ToList();
        ...
        {
            UserId = userId, UserName = userName, MachineId = machineId,
            ProjectName = projectName, ApplicationName = application,
            RevitVersion = revitVersion, StartTime = DateTime.Now, IsActive = true
        });
    }
    ...
    public void EndAllActiveSessions(string machineId, string userId)
    {
        var sessions = _cache.ActiveSessions
            .Where(s => s.MachineId == machineId && s.UserId == userId && s.IsActive).ToList();
    ...
    public void UpdateDailyRecordFromSession(WorkSession session)
    {
        ...
        var record = GetUserDailyRecord(session.MachineId, session.UserId, date)
                  ?? new DailyWorkRecord
                  {
                      Date = date, UserId = session.UserId, UserName = session.UserName,
                      MachineId = session.MachineId, FirstActivity = session.StartTime
                  };
    ...
    public void CalculateDailyTotals(DailyWorkRecord record)
    {
        ...
        var projects = record.Sessions.Select(s => s.ProjectName)
    ...
    public List<ProjectWorkSummary> GetProjectSummaries(DateTime date)
    {
        ...
        return records.SelectMany(r => r.Sessions)
            ...
            .Select(g => new ProjectWorkSummary
            {
                ...
                UserTimes = g.GroupBy(s => s.UserId)
                    ...
                    {
                        UserId = ug.Key, UserName = ug.First().UserName,
    ...
    public List<CalendarDayEntry> GetCalendarMonth(int year, int month, string? userId = null, string? machineId = null)
    {
        ...
        for (var d = first; d <= last; d = d.AddDays(1))
        {
            ...
            if (!string.IsNullOrEmpty(userId)) records = records.Where(r => r.UserId == userId).ToList();
            if (!string.IsNullOrEmpty(machineId)) records = records.Where(r => r.MachineId == machineId).ToList();
            entries.Add(new CalendarDayEntry
            {
                Date = d,
                TotalHours = records.Sum(r => r.TotalHours),
                OvertimeHours = records.Sum(r => r.OvertimeHours),
                ProjectCount = records.Sum(r => r.ProjectCount),
                HasData = records.Any(),
    ...
    {
        var today = DateTime.Today;
        var crossDay = _cache.ActiveSessions.Where(s => s.IsActive && s.StartTime.Date < today).ToList();
    ...
    public UserDailyReport GetUserDailyReport(string machineId, string userId, DateTime date)
    {
        ...
        if (record == null)
            return new UserDailyReport { UserId = userId, MachineId = machineId, Date = date };
        return new UserDailyReport
        {
            UserId = record.UserId, UserName = record.UserName, MachineId = record.MachineId,
            Date = record.Date, FirstActivity = record.FirstActivity, LastActivity = record.LastActivity,
            TotalWorkHours = record.TotalHours, RegularHours = record.RegularHours,
            OvertimeHours = record.OvertimeHours,
            ProjectBreakdown = record.Sessions.GroupBy(s => s.ProjectName)
                .Where(g => !string.IsNullOrEmpty(g.Key))
                .Select(g => new ProjectTimeEntry
                {
                    ProjectName = g.Key, Hours = g.Sum(s => s.Duration.TotalHours),
                    StartTime = g.Min(s => s.StartTime), EndTime = g.Max(s => s.EndTime),
                }).ToList()
        };
    }
    ...
    public void SaveUserDailyActivity(string userId, DateTime date, DailyActivityBreakdown breakdown)
    {
        ...
        _userDailyActivity[key] = new UserDailyActivityRecord
        {
            ...
            HourlyBreakdown = breakdown.HourlyBreakdown?
                ...
                {
                    Hour = h.Hour, RevitMinutes = h.RevitMinutes,
                    MeetingMinutes = h.MeetingMinutes, IdleMinutes = h.IdleMinutes,
    ...
    public List<string> GetUsersWithActivityOnDate(DateTime date)
    {
        ...
        return _userDailyActivity.Keys.Where(k => k.EndsWith($"|{dateStr}"))
            .Select(k => k.Split('|')[0]).Distinct().ToList();
    }

    public List<UserDailyActivityRecord> GetAllUserActivityForDate(DateTime date)
    {
        var dateStr = date.ToString("yyyy-MM-dd");
        return _userDailyActivity.Where(kv => kv.Key.EndsWith($"|{dateStr}"))
...
/// v5: in-memory only, no disk writes. Settings reset on app restart;
/// the WPF first-time-setup flow re-runs once per app launch if needed.
/// </summary>
public static class LocalStorageExtensions
{
    private static UserProfileSettings _settings = new();

    public static bool NeedsFirstTimeSetup(this LocalStorageService storage)
    {
        return !_settings.SetupCompleted || string.IsNullOrEmpty(_settings.DisplayName);
    }
    ...
    {
        var firstSetup = _settings.FirstSetupDate > DateTime.MinValue
            ? _settings.FirstSetupDate
            : DateTime.Now;
    ...
    public static string AutoCorrectAutodeskEmail(string rawEmail, string displayName = "")
    {
        ...
        var cleanName = Regex.Replace(rawEmail.ToLower(), @"[^a-z0-9]", "");

        if (!string.IsNullOrEmpty(displayName))
        {
            var parts = displayName.Trim().ToLower().Split(' ', StringSplitOptions.RemoveEmptyEntries);
            if (parts.Length >= 2)
                return $"{parts[0]}.{parts[parts.Length - 1]}@tangentlandscape.com";
        }
